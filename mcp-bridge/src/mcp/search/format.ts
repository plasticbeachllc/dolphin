...
export function buildPromptReady(res: SearchResponse, maxHits: number): string {
  ...
  for (const h of res.hits.slice(0, maxHits)) {
    ...
    if (!code) {
      parts.push(
        `No snippet included for this result. Follow up with chunk.get({chunk_id: "${h.chunk_id}"}) or file.lines({repo:"${h.repo}", path:"${h.path}", start:${h.start_line}, end:${h.end_line}}).`
      );
      continue;
    }
...
export type HitsJson = {
  ...
  hits: Array<{
    ...
    followups: {
      "chunk.get": { chunk_id: string };
      "file.lines": { repo: string; path: string; start: number; end: number };
    };
  }>;
  meta: {
    cursor: string | null;
  ...
  return {
    ...
    hits: params.hits.map((h, i) => {
      ...
      return {
        ...
        followups: {
          "chunk.get": { chunk_id: hit.chunk_id },
          "file.lines": defaultFetchLines,
        },
      };
    }),
    meta: {
