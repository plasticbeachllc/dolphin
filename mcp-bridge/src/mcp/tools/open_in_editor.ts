...
export function makeOpenInEditor (): { definition: Tool, handler: any } {
  ...
  const handler = async (args: any, signal?: AbortSignal): Promise<CallToolResult> => {
    ...
    try {
      ...
      if (!repo) throw { error: { code: 'repo_not_found', message: `Repository '${repoName}' not found` } }
      ...
      await logInfo('open_in_editor', 'open_in_editor success', { latency_ms: Date.now() - started })
      ...
      await logError('open_in_editor', 'open_in_editor error', { error_code: err.error.code, message: err.error.message })
      const content: CallToolResult['content'] = [{ type: 'text', text: `${err.error.message} Remediation: call /v1/repos and use an exact repo name.` }]
