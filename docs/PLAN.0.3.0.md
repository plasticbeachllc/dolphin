...
├── query-cache.key        # private per-install HMAC key; never stored in SQLite or diagnostics
...
Dolphin's data is local private application state, not an independently encrypted vault. SQLite databases and WAL/SHM files, LanceDB data, chunk text, embedding artifacts, manifests, logs, locks, diagnostics, temporary files, and client-configuration backups use their native formats beneath `~/Library/Application Support/Dolphin/`. There is no storage password, `DOLPHIN_STORAGE_KEY`, 1Password/Keychain lookup, SQLCipher dependency, per-artifact encryption, encryption-key hierarchy, unlock prompt, remote key escrow, or encrypted export in 0.3.0. One random 32-byte per-install `query-cache.key` is a narrowly scoped HMAC identity key, not an encryption key: SQLite stores only keyed query-cache identities, so disclosure of the metadata database alone cannot validate guessed queries or correlate them across installations. The secret is created atomically as a current-user-owned `0600` file through descriptor-relative no-follow operations and a bounded interprocess lock; it is never logged, returned, copied into SQLite, or derived from the OpenAI credential. Missing or corrupt secret state for a populated cache fails closed. Disclosure of the complete unlocked Dolphin state root, including both database and HMAC key, remains outside this narrower protection and receives no vault-like claim.
...
Path filters use one bounded canonical repo-relative glob subset over the generation's POSIX relative path. `*` and `?` never cross `/`; `**` is a complete path segment that may cross directories and also matches zero directories, with at most eight globstar segments per pattern. Matching is case-sensitive, applies to the complete path, treats ordinary punctuation literally, and gives exclusions precedence over inclusions. Absolute paths, backslashes, empty/dot/parent segments, repeated or trailing separators, embedded/consecutive globstars, NUL, oversized patterns, and oversized filter sets are rejected before query embedding. Local matching uses a deterministic non-backtracking segment automaton; the equivalent Lance predicate uses its linear-time regex engine. Language filters accept only the seven normalized first-class public names; JavaScript and TypeScript include their JSX/TSX indexed families. The exact same immutable path/language metadata is integrity-bound into SQLite membership/FTS rows and Lance vector rows, so both branches filter before their result limit and sibling worktrees interpret a filter identically without sharing authority.
...
Implementation status (2026-08-10): prerelease metadata schema v15 adds the internal query-embedding admission boundary. It computes the existing domain-separated exact-input identity in memory, derives the durable SQLite key with the separate per-install HMAC secret, validates immutable cache rows against the fixed OpenAI provider/model/dimension/contract tuple, and authenticates each keyed identity, contract field, canonical float32 vector, and creation timestamp with a domain-separated HMAC under that same protected secret. It persists neither raw/offline-guessable query identity nor query byte length, and disclosure or modification of SQLite alone cannot forge vector contents or extend cache lifetime. Rows older than 30 days are read misses without making ordinary reads mutate storage; a later live result atomically replaces the authenticated expired same-key row. First-use secret initialization is the sole bootstrap write. Cache writes use an age/key index to transactionally remove expired rows and retain at most 8,192 entries. Structural SQLite/schema/identity-secret failures are corruption rather than optional misses, while bounded contention and storage-access failures remain optional unavailability. The async OpenAI adapter reads only `DOLPHIN_OPENAI_API_KEY`, pins `text-embedding-3-small` with 1,536 float dimensions, disables SDK retries, synchronizes lazy client initialization, leases the client for each complete request/retry sequence, drains active leases before closure or credential rotation, replaces the client when the environment credential changes without retaining the raw key separately, enforces a five-second outer deadline per attempt, performs at most one short retry with bounded jitter and sanitized numeric or HTTP-date `Retry-After` handling, and maps SDK failures into closed redacted credential/transient/permanent categories. The application service moves bounded SQLite work off the event loop, shares the complete in-flight resolution—including transient degradation and cache-unavailable live results—across concurrent requests for one exact identity, and uses one event-loop-wide admission authority shared by every service instance: at most 32 distinct in-flight identities with an explicit retryable overload error, four concurrent cache calls, and four concurrent provider calls. Four of those 32 identity slots are reserved for bounded cache probes; a cache miss must atomically promote into the remaining provider-work capacity, so provider saturation cannot reject an exact cache hit while arbitrary query floods still remain globally bounded. Capacity acquisition and flight registration contain no cancellation point; after acquisition, either creation fails and synchronously releases capacity or the registered flight owns release independently of its waiters. The service checks the cache only after acquiring single-flight authority, uses exact compatible hits without a provider call, permits a correct live embedding when optional cache I/O is unavailable, and returns lexical/structural degradation only after an exhausted classified transient provider failure. Cache corruption, credentials, permanent provider failures, and provider response/model/dimension violations remain hard failures. This is an internal foundation only: public `search` remains unavailable until workspace readiness, adaptive policy, result materialization, references, and continuation authority are wired around it.
...
Generation vector storage now pins LanceDB 0.27.0 and exposes a separate strict adapter over the fixed local Application Support vector directory. Every staged row is float32-canonical, finite, nonzero, fixed at 1,536 dimensions, and bound to the exact generation chunk identity plus embedding-cache key already persisted in SQLite. One immutable table per opaque generation makes an unscoped or cross-generation backend query structurally unavailable; a domain-separated complete-row digest plus exact Lance table/version token forms the verified commit. One bounded interprocess shared/exclusive lock serializes table creation and crash reconciliation while holding committed tables stable through short SQLite visibility transitions and searches. The SQLite coordinator fully verifies that commit before recording vector readiness and again outside the write lock before readiness/publication, then performs a cheap version/schema/count stability check while holding the shared backend guard through the visibility transaction. Retrieval accepts only a live SQLite reader-lease ID, validates the exact published commit and content revision, executes fixed cosine search against that generation's table, and reauthorizes every returned chunk/cache identity through the same SQLite snapshot before returning bounded internal candidates. Exact commit verification is cached only by immutable token plus digest, while every search still rechecks backend version and live reader authority; missing, mutated, malformed, expired, or cross-scoped state fails closed.

Generation retrieval now acquires one 30-second reader lease and uses its exact published snapshot for both FTS5 and LanceDB branches, retaining the lease through deterministic fusion and revalidating it before returning. FTS5 installs an eight-second cooperative SQLite progress deadline, while every LanceDB verification or search materialization carries a ten-second backend timeout; either deadline becomes an explicit retryable retrieval timeout rather than an abandoned background read, while the keyword posting ceiling remains a separate non-retryable narrowing outcome with actionable rarer-term guidance. The fixed `generation-rrf-v1` policy independently canonicalizes each branch, rejects duplicate or over-limit identities, inspects at most 1,000 candidates per branch, applies reciprocal-rank fusion without comparing backend score scales, deduplicates before the fixed 500-target horizon, and emits only contiguous ranks plus closed branch provenance—never raw or fused scores. A deliberately omitted vector branch produces an explicit `lexical_structural` internal mode, while a successfully executed empty vector branch remains normal hybrid retrieval. Cross-store and lease failures are normalized and fail closed, and the read lease is released on every outcome. Real SQLite/FTS5/LanceDB integration coverage proves both branches share the publication and that no durable reader row remains afterward. Query-embedding/cache integration, public search wiring, ranked-plan persistence/continuation, vector GC/reachability, relevance calibration, and large-corpus policy qualification remain pending.

Search coverage admission now resolves one bounded explicit workspace set (or one already-resolved current workspace), checks the complete set before query work, and acquires exact 30-second generation reader leases only when every workspace has a published snapshot. At most eight admitted calls may own lease keepers concurrently, and each keeper has a fixed 30-second call deadline. The admitted lease set is renewed atomically every five seconds until that deadline, so bounded multi-workspace work inside the global deadline does not consume later workspaces' initial authority window without allowing an abandoned caller to pin generations indefinitely. Deadline state is carried by every admitted workspace: caller-held retrieval checks it before each backend branch and after each bounded branch, so no later branch dispatches after cancellation. A bounded 12-second drain grace retains authority for a branch already in progress before cleanup releases the leases. A missed normalized renewal can recover while the leases remain live; an unexpected coordinator failure is recorded and fails the request; deadline, expiry, or authority loss fails closed. The keeper exclusively owns post-start release, retries every lease up to three times, and continues cleanup after a bounded caller-side close timeout, preventing renew/release races. Final validation normalizes unexpected backend errors and cannot bypass keeper closure. A prior complete publication remains searchable while a newer operation runs. First-generation queued, running, and paused operations fail all-or-nothing with bounded progress; terminal operations without a publication fail closed; exceptional approval has precedence over ordinary build progress; missing, expired, inconsistent, and unclassifiable state remains unavailable. Partial pre-keeper acquisition releases every earlier lease, caller-held retrieval reuses the admitted authority without reacquiring or releasing it, and exact snapshot authority is revalidated before serialization.

First-page search execution now composes those internal foundations without enabling the public tool. Blocking coverage acquisition, final validation, and cleanup remain on a worker thread while the admitted operation runs on the caller's async event loop; cancellation is handed into that operation and does not return until the coverage owner has completed lease cleanup. Complete coverage is established before exact filtered scope resolution and the one query-embedding call. Scope resolution proves bounded per-workspace and aggregate searchable-chunk counts from live validated publications before provider work; an aggregate zero returns an exact empty `not_needed` plan without embedding or retrieval, and zero-count members of a nonempty multi-workspace scope skip both retrieval backends. Cancellation drains that bounded SQLite read before coverage release. Canonical path/exclusion/language scope is applied identically to FTS5 candidates and as an integrity-bound Lance prefilter before vector top-k, so an ineligible nearest vector cannot hide an eligible result. One bounded shared execution service then dispatches at most eight workspace retrievals concurrently. Cancellation or the first workspace failure prevents capacity-waiting workspaces from starting while retaining authority until backend calls that already started finish or reach their existing deadline, so no background FTS5/LanceDB read outlives its reader authority. Generation retrieval can return a deliberately non-representable transient candidate container whose scores exist only for immediate in-process fusion. The global `search-global-rrf-v1` policy independently canonicalizes every admitted workspace's candidates using only each index's local scores, deterministically round-robins equal local ranks in canonical workspace order to form one keyword branch and one optional vector branch without comparing scores across indexes, and then combines only reciprocal ranks across those two global branches. It preserves identical chunks from sibling worktrees as distinct exact workspace-publication-generation targets, deduplicates before one global 500-target horizon, and emits an eager immutable plan with contiguous rank, closed provenance, scope digest/count/shape, embedding/degradation metadata, and no raw score or query identity. Coverage, scope, embedding, branch, and plan failures remain fail-closed and all-or-nothing. Adaptive output budgets, relevance calibration, snippet/reference materialization, continuation persistence, cleanup-intent admission, MCP error projection, and `search` availability remain pending.
