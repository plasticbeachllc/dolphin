...
Implementation status (2026-08-10): the internal query-embedding admission boundary now computes the existing domain-separated exact-input identity, validates immutable SQLite cache rows against the fixed OpenAI provider/model/dimension/contract tuple, stores only identity metadata plus canonical float32 vector bytes and their digest, and keeps raw query text out of durable cache state. The async OpenAI adapter reads only `DOLPHIN_OPENAI_API_KEY`, pins `text-embedding-3-small` with 1,536 float dimensions, disables SDK retries, enforces a five-second outer deadline per attempt, performs at most one short retry, and maps SDK failures into closed redacted credential/transient/permanent categories. The application service moves bounded SQLite work off the event loop, uses exact compatible cache hits without a provider call, permits a correct live embedding when optional cache I/O is unavailable, and returns lexical/structural degradation only after an exhausted classified transient provider failure. Cache corruption, credentials, permanent provider failures, and provider response/model/dimension violations remain hard failures. This is an internal foundation only: public `search` remains unavailable until workspace readiness, adaptive policy, result materialization, references, and continuation authority are wired around it.
