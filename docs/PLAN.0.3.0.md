...
| D-037 | `repo_add` accepts only the absolute concrete worktree path. Dolphin derives deterministic human-readable repository/workspace display labels and disambiguates duplicates with stable short IDs; names are never caller-supplied identity, scope, or conflict keys. |
| D-038 | Git submodule working trees are excluded from their parent workspace and require their own explicit path-only `repo_add` when needed. Parent enrollment reports submodule state and ready-to-use enrollment actions but never initializes, updates, recursively registers, embeds, or implicitly searches submodule contents. |
...
| D-069 | The 0.3.0 cleanup UX is frozen at one optional `CleanupAuthority` output from path-only `repo_add` and one two-field `repo_forget(workspace_id, cleanup_receipt)` input. The receipt is opaque. There are no agent-facing temporary/persistent, wait, cancel, force, recursive, source/worktree-delete, GC, TTL, retention, victim, recovery, or tuning controls; all internal cleanup defaults are release-defined, and results clearly distinguish immediate logical release from asynchronous physical reclamation. |
...
The canonical guidance also teaches capability-scoped cleanup: when `repo_add` returns a cleanup receipt, retain it for the task and call `repo_forget` if that exact workspace registration is no longer needed. `cleanup = null` means this call did not create the registration epoch and has no cleanup authority. This is not an agent/human or temporary/persistent repository classification. Cleanup releases only Dolphin's registration and derived state; deleting a Git worktree remains outside Dolphin.

### 5.3 First repository flow

Repository enrollment is an MCP workflow. Neither `dolphin setup`, `dolphin doctor`, process startup, the current working directory, nor any human CLI indexing command may create or reactivate a registration; the sole public creation path is the agent's explicit path-only `repo_add` call.
...
5. Dolphin performs a bounded preflight, starts indexing without a separate approval prompt, and returns a structured operation identifier plus immediately useful status. If this call created the registration, it also returns the only agent cleanup receipt for that registration epoch. Only an approvable aggregate catastrophic-fuse stop pauses in `awaiting_approval` with explicit human remediation.
...
2. If the client exposes MCP roots, exactly one root that resolves to a registered worktree selects that workspace. Nested subdirectories resolve to their containing worktree.
...
- In the transaction that creates a new active workspace registration, Dolphin generates at least 256 bits of cryptographically secure random material, encodes it as a versioned `dolphin-cleanup-v1_...` receipt, and binds a domain-separated receipt digest to the workspace ID and a random registration epoch.
- Exactly one winner of concurrent `repo_add` creation receives the receipt. A call that discovers an existing registration returns `cleanup = null`, even if the caller supplied the same path or caused a sync. Repeated enrollment can never acquire cleanup authority for a pre-existing workspace.
- The raw receipt is returned only in the successful creation result. It is excluded from `repo_list`, `status`, ordinary `operation_status`, logs, metrics, errors, generated examples, and analytics. SQLite stores the digest and lifecycle metadata, not a recoverable raw receipt. Loss of the receipt requires human cleanup; there is no MCP receipt listing, rotation, or recovery operation.
...
Concurrent `repo_add` calls serialize the identity decision. Exactly one call creates the fresh epoch and receives its cleanup receipt; later calls observe that active registration, return the same workspace ID/epoch with `cleanup = null`, and cannot start a second first-index operation. A concurrent old-receipt replay can return only the old epoch's `already_forgotten` result and has no locking or authorization path to the fresh epoch.
...
1. Call path-only `repo_add` as normal.
...
- `cleanup` is present only for the call that won creation of a new registration epoch. An existing registration always returns `cleanup = null`; retrying `repo_add` is not a way to acquire cleanup authority.
- The raw cleanup receipt is excluded from logs and every listing/status response. Losing it leaves the registration usable but requires the human CLI to forget it.
...
def mint_cleanup_receipt(workspace_id: str, registration_epoch: str) -> tuple[str, bytes]:
    receipt = CLEANUP_RECEIPT_PREFIX + secrets.token_urlsafe(CLEANUP_RECEIPT_BYTES)
    digest = cleanup_receipt_digest(workspace_id, registration_epoch, receipt)
...
Production parsing first enforces the exact prefix and bounded base64url payload shape, then performs `secrets.compare_digest` against the stored digest. Issuance and the registration insert share one SQLite transaction. Forget uses the lifecycle-lock/revalidation/consume transaction from Section 5.34; this helper does not itself authorize deletion. The registration epoch is a random opaque ID, and neither it nor a workspace ID substitutes for the receipt.
...
- receipt-authorized cleanup of a workspace registration, including correct receipt retention/use, refusal without the creation result's receipt, and proof that Git/source state remains unchanged;
...
- Make `RepoAddInput` path-only; derive deterministic display labels and disambiguate presentation collisions with stable short IDs.
...
- [ ] Mint a cleanup receipt only inside the winning new-registration transaction; store its domain-separated digest, return the raw value only in that creation result with `expires_at = null`, and prove repeat/concurrent `repo_add` calls receive no authority or different registration kind.
...
- [ ] Teach agents to retain a returned cleanup receipt for the task, call `repo_forget` explicitly only when that exact registration is genuinely no longer needed, treat `cleanup = null` as no removal authority, never treat disconnect/session end/process exit as disposal, and defer receipt-lost cleanup to the human CLI without teaching agent/human or temporary/persistent registration classes.
...
- [ ] Ensure generated instructions and both client adapters teach path-only MCP `repo_add` as the sole enrollment path, never suggest a CLI alternative, and use returned stable workspace IDs for explicit scope.
...
- [ ] Add mirrored Codex/Claude examples for worktree enrollment, delta reuse, receipt-authorized cleanup, `WORKSPACE_IN_USE` retry, and refusal when `cleanup = null`; assert the receipt never appears as a literal example value and no example labels registrations by human/agent ownership or temporary/persistent class.
- [ ] Keep cleanup guidance to the canonical four-step flow and prove generated clients expose no cleanup choice or argument beyond retaining the opaque creation result and later passing its workspace ID/receipt to `repo_forget`.
...
17. release the exact workspace-registration epoch whose creation result supplied its receipt through the two-field cleanup contract, while being unable to release an epoch for which it has no receipt and leaving the Git worktree, source files, and shared derived artifacts untouched; and
