...
Search coverage admission now resolves one bounded explicit workspace set (or one already-resolved current workspace), checks the complete set before query work, and acquires exact 30-second generation reader leases only when every workspace has a published snapshot. At most eight admitted calls may own lease keepers concurrently, and each keeper has a fixed 30-second call deadline. The admitted lease set is renewed atomically every five seconds until that deadline, so bounded sequential multi-workspace work does not consume later workspaces' initial authority window without allowing an abandoned caller to pin generations indefinitely. A missed normalized renewal can recover while the leases remain live; an unexpected coordinator failure is recorded and fails the request; deadline, expiry, or authority loss fails closed. The keeper exclusively owns post-start release, retries every lease up to three times, and continues cleanup after a bounded caller-side close timeout, preventing renew/release races. A prior complete publication remains searchable while a newer operation runs. First-generation queued, running, and paused operations fail all-or-nothing with bounded progress; terminal operations without a publication fail closed; exceptional approval has precedence over ordinary build progress; missing, expired, inconsistent, and unclassifiable state remains unavailable. Partial pre-keeper acquisition releases every earlier lease, caller-held retrieval can reuse the admitted lease without reacquiring or releasing it, and exact snapshot authority is revalidated before serialization. This remains an internal foundation: public `SCOPE_FUSE_TRIPPED`/`INDEX_BUILDING` projections, cleanup-intent admission, adaptive policy, query-embedding orchestration, materialization, references, and continuation authority remain pending, so the public `search` tool is still unavailable.
