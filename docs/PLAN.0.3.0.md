...
Search coverage admission now resolves one bounded explicit workspace set (or one already-resolved current workspace), checks the complete set before query work, and acquires exact 30-second generation reader leases only when every workspace has a published snapshot. The admitted lease set is renewed atomically every five seconds for the lifetime of the search call, so bounded sequential multi-workspace work does not consume later workspaces' authority window; a missed normalized renewal can recover while the leases remain live, an unexpected coordinator failure is recorded and fails the request, and final exact-snapshot validation still fails closed after expiry or authority loss. A prior complete publication remains searchable while a newer operation runs. First-generation queued, running, and paused operations fail all-or-nothing with bounded progress; terminal operations without a publication fail closed; exceptional approval has precedence over ordinary build progress; missing, expired, inconsistent, and unclassifiable state remains unavailable. Partial lease acquisition releases every earlier lease, caller-held retrieval can reuse the admitted lease without reacquiring or releasing it, and exact snapshot authority is revalidated before result serialization. This remains an internal foundation: public `SCOPE_FUSE_TRIPPED`/`INDEX_BUILDING` projections, cleanup-intent admission, adaptive policy, query-embedding orchestration, materialization, references, continuation authority, and a shared/coalescing renewal scheduler or equivalent explicit public-search admission limit remain pending, so the public `search` tool is still unavailable.
