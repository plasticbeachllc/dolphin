...
Generation staging now also derives one exact FTS5 keyword document from each verified artifact in the same transaction as membership and manifest persistence. A domain-separated keyword commit binds the generation, content manifest, artifact identity, path, language, and item count; readiness and publication recompute that binding before marking its revision validated. Lexical retrieval accepts only bounded literal terms plus an unexpired reader-lease ID, resolves the published generation entirely from SQLite, filters every candidate to that generation, and rejects any post-validation keyword mutation. Staging and ready keyword rows remain invisible. LanceDB generation commits, vector retrieval, hybrid fusion, public search wiring, and GC reachability remain pending.
