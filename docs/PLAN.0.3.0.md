...
Path filters use one bounded canonical repo-relative glob subset over the generation's POSIX relative path. `*` and `?` never cross `/`; `**` may cross directories and `**/` also matches zero directories. Matching is case-sensitive, applies to the complete path, treats ordinary punctuation literally, and gives exclusions precedence over inclusions. Absolute paths, backslashes, empty/dot/parent segments, repeated or trailing separators, NUL, oversized patterns, and oversized filter sets are rejected before query embedding. Language filters accept only the seven normalized first-class public names; JavaScript and TypeScript include their JSX/TSX indexed families. The exact same immutable path/language metadata is integrity-bound into SQLite membership/FTS rows and Lance vector rows, so both branches filter before their result limit and sibling worktrees interpret a filter identically without sharing authority.
