...
Native Apple Silicon storage qualification now exercises Application Support initialization and immutable artifact installation through real macOS filesystem syscalls. Directory entries are synced where the host filesystem supports it, only explicit unsupported-operation errors degrade gracefully, and repeated artifact puts verify the existing immutable envelope under the install lock before allocating or syncing temporary bytes.
