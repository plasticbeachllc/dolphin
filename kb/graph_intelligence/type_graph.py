...
class TypeGraphExtractor:
    ...
    def extract_type_relationships_python(
        self, tree_root: Node, file_path: str, repo_id: int, file_id: int,
        commit_sha: str, existing_classes: List[GraphNode]
    ) -> List[GraphEdge]:
    ...
    def extract_type_relationships_typescript(
        ...
        for node, capture_name in captures:
            if capture_name in ("class_name", "class_name2"):
                ...
                interface_node = next(
                    (cls for cls in existing_classes
                     if cls.name == interface_name and cls.node_type == NodeType.CLASS),
                    None
                )
