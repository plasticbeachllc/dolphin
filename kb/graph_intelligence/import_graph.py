...
class ImportGraphExtractor:
    ...
    def extract_imports_python(
        self, tree_root: Node, source_file: str, file_id: int,
        repo_id: int, commit_sha: str, all_files: List[str]
    ...
    def extract_imports_typescript(
        ...
        for node, capture_name in captures:
            if capture_name == "import_source":
                ...
                if target_file:
                    # Create target module node
                    target_module_name = target_file.replace('/', '.').replace('.ts', '').replace('.tsx', '').replace('.js', '')
                    target_module_node = GraphNode(
                        id=str(uuid.uuid4()),
                        repo_id=repo_id,
                        node_type=NodeType.MODULE,
    ...
    def _resolve_python_import(self, import_path: str, all_files: List[str]) -> Optional[str]:
