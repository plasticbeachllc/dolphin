...
def _load_global_extension_map() -> dict[str, str]:
    ...
    try:
        ...
        _log.warning(
            "Failed to load extension mappings: %s. Language detection will be disabled.",
            e,
        )
        return {}
...
def chunk_file(
    *,
    ...
    _log.info(
        ...
        rel_path,
        len(enriched_chunks),
        (sum(c.token_count for c in enriched_chunks) // len(enriched_chunks) if enriched_chunks else 0),
    )
