...
async def index_files(request: IndexRequest) -> IndexResponse:
    ...
    This endpoint accepts a list of file paths and indexes only those files,
    leveraging SHA256 deduplication to skip unchanged content.
    """
    if _sql_store is None or _lance_store is None:
        raise HTTPException(status_code=503, detail="Stores not initialized")
    ...
    # Use the pipeline to index the files
    # We'll create a temporary method to process specific files
    try:
        import subprocess
