...
const testCases = [
  ...
  { path: '../dolphin/package.json', expected: true, description: 'Path that resolves to workspace' },
  { path: '../../etc/passwd', expected: false, description: 'Directory traversal outside workspace' },
  { path: '/etc/passwd', expected: false, description: 'Absolute path outside workspace' },
  { path: '/home/user/dolphin/package.json', expected: true, description: 'Absolute path within workspace' },
...
testCases.forEach(({ path: testPath, expected, description }) => {
  ...
  if (success) {
    ...
    console.log(`   Path: ${testPath}`);
    console.log(`   Expected: ${expected ? 'Allow' : 'Block'}, Got: ${result.allowed ? 'Allow' : 'Block'}`);
    if (result.resolvedPath) console.log(`   Resolved: ${result.resolvedPath}`);
