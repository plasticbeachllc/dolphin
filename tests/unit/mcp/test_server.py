...
from kb.mcp.server import _complete_handlers, _wire_tool, create_server
...
async def test_server_validates_before_invoking_a_handler() -> None:
    ...
    def status_handler(_input: StatusInput) -> dict[str, str]:
        nonlocal called
        called = True
        return {"readiness": "ready"}

    server = create_server({"status": status_handler})
    handler = server._request_handlers["tools/call"].handler
    result = await handler(None, mcp_types.CallToolRequestParams(name="status", arguments={"unexpected": True}))
...
async def test_server_returns_structured_success_and_bounded_not_ready_errors() -> None:
    server = create_server({"status": lambda _input: {"readiness": "ready"}})
    handler = server._request_handlers["tools/call"].handler

    success = await handler(None, mcp_types.CallToolRequestParams(name="status", arguments={}))
    not_ready = await handler(None, mcp_types.CallToolRequestParams(name="repo_list", arguments={"cursor": None}))

    assert success.is_error is False
    assert json.loads(success.content[0].text) == {"readiness": "ready"}
...
async def test_server_can_call_the_real_status_application_service(tmp_path) -> None:
    server = create_server({"status": StatusService(cwd=tmp_path, environment={})})
    handler = server._request_handlers["tools/call"].handler

    result = await handler(None, mcp_types.CallToolRequestParams(name="status", arguments={}))
