...
def pipeline_with_fts(tmp_path: Path) -> IngestionPipeline:
    ...
    config = KBConfig(store_root=str(store_root), embedding_provider="openai")
...
async def test_fts_indexing_called_in_parallel_mode(pipeline_with_fts, sample_repo):
    ...
    pipeline_with_fts.metadata.record_repo(
        name="test_repo", path=sample_repo, default_embed_model="small"
    )
    ...
    # Run parallel index
    result = await pipeline_with_fts.index_parallel(
        "test_repo", dry_run=False, full_reindex=True, max_workers=2
    )
