...
def test_real_coverage_keyword_and_vector_stores_share_one_published_snapshot(
    ...
    root = _directory(tmp_path / "repository")
    ...
    worktree = GitWorktree(
        ...
        common_git_dir_identity="common-generation-retrieval",
        worktree_git_dir=root / ".git",
        worktree_git_dir_identity="worktree-generation-retrieval",
        head_commit="a" * 40,
        branch="develop",
    )
    registration, _operation = registry.register_and_submit_initial_index(
        worktree,
        cleanup_receipt=_cleanup_receipt("generation-retrieval"),
    )
    runtime = registry.register_runtime(
        runtime_id="runtime_generation_retrieval",
        pid=105,
        process_start_identity="start-generation-retrieval",
        mode="mcp",
    ...
    lexical = _membership(artifacts, "lexical", "needle lexical evidence")
    semantic = _membership(artifacts, "semantic", "conceptual semantic evidence")
