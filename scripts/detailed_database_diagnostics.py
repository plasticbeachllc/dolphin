...
def check_all_database_locations():
    ...
    for path in potential_paths:
        ...
        if path.exists():
            ...
            if meta_db.exists():
                print(f"   ✅ metadata.db exists")
            else:
                print(f"   ❌ metadata.db not found")
            
            # Check for LanceDB data
            lancedb_dir = path / "_lancedb"
            if lancedb_dir.exists():
                print(f"   ✅ LanceDB directory exists")
                # Check for tables
...
def test_vector_search_detailed():
    ...
    try:
        ...
        if results:
            ...
            print(f"    - Tables are empty")
            print(f"    - Vector dimensions don't match")
            print(f"    - Query vector is too different from indexed vectors")
...
def main():
    ...
    print(f"\n💡 Summary:")
    print(f"If you have indexed repositories but see empty results:")
    print(f"1. Check that the repository indexing completed successfully")
    print(f"2. Verify the LanceDB tables contain vector data")
    print(f"3. Ensure vector dimensions match between indexing and search")
